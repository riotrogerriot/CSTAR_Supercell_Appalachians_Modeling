...
os_type = 1
if( os_type == 0 ):
    wdir = '/Users/roger/Library/CloudStorage/OneDrive-UniversityofNorthCarolinaatCharlotte/CSTAR_Modeling_Project/Simulations/Steady_State_Sims/id_terrain/'
else:
    wdir = 'C:/Users/rriggin/OneDrive - University of North Carolina at Charlotte/CSTAR_Modeling_Project/Simulations/Steady_State_Sims/id_terrain/'
...
#------------------------------------------------------------------------------
sim = [ 'cs_ctl' , 'cs_trn', 'cs_mod', 'nc_ctl', 'nc_trn', 'nc_mod' ]                 # Sim names
color = [ 'darkorange', 'tab:red', 'maroon', 'lightskyblue', 'tab:blue', 'navy' ]     # Sim colors
sup_start = [ 23, 25, 21, 28, 23, 26 ]                                                # Supercell Start Index
sup_end = [ 51, 52, 51, 67, 54, 48 ]                                                  # Supercell End Index
dis_end = [ 61, 68, 61, 72, 72, 71 ]                                                  # Dissipation Time Index
linear = [ 0, 0, 0, 71, 71, 71 ]                                                      # Upscale Growth Index
#------------------------------------------------------------------------------
...
ax_A.set_title( 'a: IDTRN Steady-State "Crossing" Environment', fontsize = 18, fontweight = 'bold', loc = 'left' )
ax_A.set_xlabel( '', fontsize = 18, fontweight = 'bold' )
ax_A.set_ylabel( 'Meridional Distance (km)', fontsize = 18, fontweight = 'bold' )
ax_A.set_xlim( x1, x2 )
ax_A.set_ylim( 150, 300 )
...
ax_B.set_title( 'b: IDTRN Steady-State "Non-Crossing" Environment', fontsize = 18, fontweight = 'bold', loc = 'left' )
ax_B.set_xlabel( '', fontsize = 18, fontweight = 'bold' )
ax_B.set_ylabel( 'Meridional Distance (km)', fontsize = 18, fontweight = 'bold' )
ax_B.set_xlim( x1, x2 )
ax_B.set_ylim( 50, 200 )
...
ax_C.set_title( 'c: IDTRN Cross-Section', fontsize = 18, fontweight = 'bold', loc = 'left' )
...
for i in range( 0, len( sim ) ):
    ...
    # Variable axis logic( i < 2 = CS in Panel A, i > 2 = NC in Panel B )
    if( i > 2 ):
        ax_i = 1
    ...
    filename2 = wdir + '/Stats_Spreadsheets/IDTRN_Final_Output_Files/' + sim[i] + '_cm1out_000073.nc'
...
ax_A.legend( title = 'Storm Mode (Start/End)', handles = [sup_lab, lin_lab], loc = 'upper right', facecolor = 'lightgrey', fontsize = 14 )
...
# Save figure
fig.savefig( fname = 'IDTRN_storm_tracks_w_swath.jpeg', dpi = 300, bbox_inches = "tight" )

...
terr_file = wdir + 'Originals/Stats_Spreadsheets/Realistic_Terrain_Track_Swath/cm1out_000097_i.nc' 
...
x1 = 120            # X-axis min (km)
x2 = 355            # X-axis max (km)
...
for i in range( 0, len( sim ) ):
    ...
    uhx_smooth = gauss_smoother( uhx, 1.0 )
    uhy_smooth = gauss_smoother( uhy, 1.0 )
    ...
    filename2 = wdir + 'V3/' + sim[i] + '/' + sim[i] + '_cm1out_000097.nc' 
    ...
    zs_alt = gauss_smoother( zs * 1000, 0.5 )
    ...
    #------------------------------------------------------------------------------
    if( linear[i] > 0 ):
        ax_B.scatter( uhx_smooth[ sup_end[i]+1 ], zs_alt[ sup_end[i]+1 ], marker = 's', color = color[i], edgecolor = 'k', s = 100, zorder = 10 )
...
# Save figure
fig.savefig( fname = 'RLTRN_storm_track_w_swath_V2_python_3_11.jpeg', dpi = 300, bbox_inches = "tight" )

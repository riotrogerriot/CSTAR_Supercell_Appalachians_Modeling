...
This folder contains all necessary files (e.g., CM1 executables, input_soundings, and namelists) required to re-run each steady-state (Base-State stems from Upstream Composites only; Fig. 3a,d; Riggin IV et al. In Review) simulation. Each simulation folder also includes a onefile.F for reference and a 5_Min_Output folder which contains visual references generated by our tracking algorithm (/Scripts/Tracking_Algorithm) that were discussed in the Publication but again not shown for brevity. 
